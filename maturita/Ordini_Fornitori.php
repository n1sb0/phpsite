...
<html>
    ...
    <div class="nav__menu" id="nav-menu">
        <ul class="nav__list">
          ...
          <li class="nav__item"><a href="Ordini_Fornitori.php" class="nav__link active">Ordini Fornitori</a></li>
        </ul>
      </div>
    ...
    <?php
        ...
        $sql = "SELECT * FROM ordine_fornitore";

        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            echo "<center>";
            echo "<table><tr><th>ID</th><th>Codice fornitore</th><th>Codice prodotto</th><th>quantità</th>";
            //crea in output una riga con 3 colonne per ciascun record
            while($row = $result->fetch_assoc()) {
                echo "<tr>
                        <td>" . $row["id"]. "</td>
                        <td>" . $row["codice_fornitore"]. "</td>
